...
其中 $\kappa(W) = \sigma_{\max}/\sigma_{\min}$ 是权重��阵的条件数。这意味着：**量化引起的输出误差上界由条件数线性控制**。
...
对于 FP4（$u \approx 0.0625$）的 $n = 10^9$ 次梯度累加：确定性误差量级 $\sim 6.3 \times 10^7$，随机舍入误差量级 $\sim 2000$——**降低 4 个数量级**。
...
- **验证 2**：RMSNorm ���融实验——移除 RMSNorm vs 保留 RMSNorm，测量误差传播差异
