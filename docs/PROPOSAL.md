...
> - [`REPORT.md`](REPORT.md) — 最终实验报告（18 配置 PTQ 对比、定理 1′ 分量级验证）
> - [`THEOREM.md`](THEOREM.md) — 全部定理推导（含定理 1′ 分量级界）
> - [`ANALYSIS.md`](ANALYSIS.md) — 实验设计审查及修复记录
>
> **关键方向修正：**
> - **PPL 已完全弃用。** 所有实验使用 `||dy||/||y||`（线性层输出空间相对误差）作为唯一评估指标。下文所有原始设计中的 PPL 均已被替换为 `||dy||/||y||` 或 loss。
> - **定理 1（范数界 $\kappa(W)$）已被实验证伪**（r = −0.16, p = 0.15）
> - **定理 1′（分量级界 $\text{cond}_{\text{cw}}(W,x)$）已验证**（r = 0.928, p = 8.0×10⁻¹¹³, r² = 0.861）
...
- **验证 1**：单层量化实验——逐层量化并测量实际 `||dy||/||y||`，与 κ(W) 预测对比（原始设计：PPL 退化，已弃用）
...
| 条件数正则化损害训练收敛 | 网格搜索 λ ∈ {0, 1e-5, 1e-4, 1e-3}，选择 `||dy||/||y||` 退化 < 10% 的最大 λ（原始设计：PPL 退化） |
...
| 实验 | 方法 | 度量 | 通过标准 |
|------|------|------|----------|
| 逐层量化敏感性 | 每次仅量化一层，测量 `||dy||/||y||` | Pearson r(κ, `||dy||/||y||`) | r = −0.16（已被分量级界取代） |
| RMSNorm 消融 | 移除/保留 RMSNorm 下的误差传播对比 | 阻断比 Δloss_no/Δloss_yes | > 100×（已验证：~1221×） |
...
| 指标 | 定义 | 用途 |
|------|------|------|
| `||dy||/||y||` | $\|\delta y\|/\|y\|$（线性层输出空间相对误差） | 总体量化保真度（已替代原 PPL 退化指标） |
