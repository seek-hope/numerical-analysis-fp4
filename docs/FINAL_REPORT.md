...
本项目以一个 ~164M 参数的 Gemma 风格因果 Transformer 为载体，系统研究 FP8/FP4 权重量化中的误差行为，通过实证手段检验数值分析中的经典理论工具。研究围绕理论、工程与算法三个维度递进展开。实验发现，逐分量条件数相比规范条件数界能更好预测FP4误差；单层条件下随机舍入和条件数正则化方法反而可能增大误差；并验证了 RMSNorm、κ加权Lloyd-Max 等思想的有效性。研究在FP4、小规模模型的场景下，尝试将理论与实践结合，为低精度量化提供新的视角与评估工具。
...
> 针对研究问题3，在本研究中我们提出两个可能的策略：（A） κ 加权 Lloyd-Max 量化器；（B）条件数正则化。

### 2.6 策略 A κ 加权 Lloyd-Max
...
格式覆盖 FP8 E4M3 与 FP4 E2M1 两种。两个检查点 × 两种格式 × 多种方法，共计 18 配置。
...
### 4.3 定理 3 验证：随机舍入的前向误差

定理 3 预测：确定性舍入的累积误差为 $O(n \cdot u)$（最坏情况），随机舍入（无偏、每次独立）则为 $O(\sqrt{n} \cdot u)$（期望 $L^2$ 范数）。对于 FP4 E2M1（$u = 0.25$）、$n = 10^9$ 的梯度累积，随机舍入理论上可将误差减少约 $3.16 \times 10^4$ 倍。我们在同一组权重矩阵和 FP16 激活上，分别对比 RTN（确定性最近邻舍入）与 `rtn_sr`（随机舍入），测量单层输出相对误差 $\|\delta y\|/\|y\|$。得到结果如下：

| Checkpoint / Format    | RTN Mean | rtn_sr Mean | $\Delta$  |
| ---------------------- | -------- | ----------- | --------- |
| fp16_baseline / FP8    | 0.01367  | 0.01933     | **+41%**  |
| fp16_baseline / FP4    | 0.06117  | 0.08645     | **+41%**  |
| cond_regularized / FP8 | 0.01412  | 0.01996     | **+41%**  |
| cond_regularized / FP4 | 0.06307  | 0.08913     | **+41%**  |

**分析。** 随机舍入在所有配置上一致地使前向误差增大 41%，与定理 3 的预测方向相反。原因有两层：

1. 定理 3 的适用域是累积求和，而非单层权重误差。定理 3 的 $O(\sqrt{n} \cdot u)$ 优势来自 $n$ 个独立无偏舍入的随机游走抵消，成立前提是误差被累积叠加（如长序列点积）。单层 $y = Wx$ 的相对误差由 $\|\delta W \cdot x\|/\|Wx\|$ 决定——分子中每个权重都独立引入有界随机噪声，但此时分母 $\|Wx\|$ 不随舍入样本变化；随机舍入的无偏性使期望误差接近 RTN，但每次实现的随机波动反而增大了单次测量的 $\|\delta y\|$。

2. FP4 的格点间距 $u = 0.25$ 决定了误差下限。在格点仅有 16 个的极稀疏场景下，随机舍入的噪声方差比确定性舍入的误差方差更大，因此可能带来更大误差。

3. STE 梯度在 QAT 中以 FP16/FP32 累积，定理 3 的 $\sqrt{n}$ 优势仅存在于前向传播的长累积路径中。本实验度量的是 PTQ 前向误差，而非 QAT 梯度累积误差，因此随机舍入的理论优势不适用。

**结论。** 定理 3 在理论上正确，但在这里单层向前误差的评估场景下，随机舍入的误差优势并不明显，甚至会导致误差增大。

### 4.4 定理 4 Lloyd-Max 优于 uniform E2M1
...
| ---------------- | ------------ | -------------------------- | ----------------------------- |
| fp16_baseline    | uniform E2M1 | 0.0612                     | 0.0593                        |
| fp16_baseline    | Lloyd-Max    | **0.0664**                 | **0.0640**                    |
|                  |              | $+8.6\%$                   | $+8.0\%$                      |
| cond_regularized | uniform E2M1 | 0.0631                     | 0.0618                        |
| cond_regularized | Lloyd-Max    | **0.0680**                 | **0.0660**                    |
|                  |              | $+7.9\%$                   | $+6.8\%$                      |

Lloyd-Max 的相对表现相对于 RTN 略低，但是 Lloyd-Max 仍然有效，自适应格点优于固定格点，但优势被 RTN 的对数间距所部分抵消。

**机制分析。** FP4 上 RTN 的 $\|\delta W\|/\|W\|$ 几乎被 $u=0.25$ 的间隔钉死；Lloyd-Max 通过把 16 个层级移动到经验权重分布的密集区，直接缩小了 $\|\delta W\|$ 本身——而非借助 $\kappa$ 通道。E2M1 的对数间距对 Transformer 权重的近似对数正态分布已有较好覆盖，故 Lloyd-Max 改善幅度有限。

### 4.5 策略 A 验证：κ 加权 Lloyd-Max

策略 A（κ 加权 Lloyd-Max，定理见 2.6 节）在标准 Lloyd-Max 的失真泛函中引入 $\kappa$ 相关的重要性权重，为高条件数矩阵的大权重分配更多量化精度。类似于 4.4 节的实验设置，我们对比了 `lloyd_max` 与 `lloyd_max_kappa` 两种方法：

| Checkpoint       | lloyd_max | lloyd_max_kappa | $\Delta$  |
| ---------------- | --------- | --------------- | --------- |
| fp16_baseline    | 0.06643   | 0.06212         | **−6.5%** |
| cond_regularized | 0.06804   | 0.06375         | **−6.3%** |

两个检查点均降低约 6.4%，策略 A 得到验证。机制上，$\kappa$ 此处并非作为预测指标，而是将格点偏向大幅值权重，直接减少对输出线性贡献最大的分量的量化误差。

### 4.6 策略 B 验证：条件数正则化

策略 B（条件数正则化，定理见 2.7 节）的详细讨论讲在下一章中完成。

## 五、工业方案基准测试

本章在统一平台上系统对比各个量化配置在 FP8 与 FP4 两种格式、两个检查点上的表现。所有方法在同一组单遍捕获的 FP16 激活上量化，使用同一组验证集激活做评估。
...
FP8 排名：rtn < rtn_sr < gptq；FP4 排名：rtn < lloyd_max_kappa < lloyd_max < mxfp4 < rtn_sr < gptq

值得注意的是，Mean 与 Total 两个指标对所有有效方法给出完全相同的排名，说明等权指标低估 GPTQ 效果的顾虑可以排除。

### GPTQ 与 RTN 的比较

| Checkpoint / Format    | RTN Mean | GPTQ Mean | $\Delta$     |
| ---------------------- | -------- | --------- | -------------- |
| fp16_baseline / FP8    | 0.01367  | 0.02040   | **+49%** |
| fp16_baseline / FP4    | 0.06117  | 0.09013   | **+47%** |
| cond_regularized / FP8 | 0.01412  | 0.02108   | **+49%** |
| cond_regularized / FP4 | 0.06307  | 0.09284   | **+47%** |

Total $\|\Delta WX\|/\|WX\|$ 的对比同样给出 +50% 量级的劣化。GPTQ 把矩阵按列序贪心量化，每列量化后将剩余列做线性补偿以抵消已发生的舍入误差：这与先前研究的结果有所不同，在本研究的场景下我们给出如下分析：
...
补偿信号本身也受 FP4 格点限制——补偿量被舍入回格点，残差累积；最后一列没有"剩余列"可补偿，必须独自吸收所有累积补偿误差。在小模型（164M）+ 强量化（FP4）的场景下，列序补偿引入的噪声超过了它能消除的原始舍入误差。GPTQ 在 LLaMA-7B 等大模型上有效的前提（列数充裕、激活长尾稀疏）在本设置下不成立。
...
| FP8 / gptq           | 0.02040       | 0.02108          | +3.3%      |
| FP4 / rtn            | 0.06117       | 0.06307          | +3.1%      |
| FP4 / lloyd_max      | 0.06643       | 0.06804          | +2.4%      |
| FP4 / lloyd_max_kappa| 0.06212       | 0.06375          | +2.6%      |
| FP4 / gptq           | 0.09013       | 0.09284          | +3.0%      |

条件数正则化的初衷是主动降低 $\kappa(W)$ 以让权重对量化更友好，但在所有方法上均使误差变差约 3%。在当前的实验环境下，条件数正则化并不是一个好的策略，因为瓶颈与格点间隔，而非奇异值分布相关。

综上，对于本实验中的 FP4 量化，RTN 是最优选择，其次是 lloyd_max_kappa；Lloyd-Max 次之。
...
本实验验证了定理 2、定理 4 和策略 A，对定理 1 和定理 3 给出了可能不适用的原因及适用的条件。通过各个量化方法的横向比较，揭示了 GPTQ、随机舍入和条件数正则化的局限，并验证了 RTN 与 Lloyd-Max 量化方法在这一场景下的真实效果。

我们从中归纳出在真实训练中，对实践的指导建议：

- FP8 场景：建议使用 RTN。FP8 的量化误差本身已经很小，Hessian 加权补偿引入的扰动反而更大。
- FP4 场景：首选 RTN；若需自适应网格，用 lloyd_max_kappa。
- 用随机舍入做 PTQ时，需要谨慎考虑。随机舍入的 $O(\sqrt{n}\cdot u)$ 优势仅在长程梯度累积路径中成立，对单层前向误差反而引入额外噪声。
- 不建议用条件数 $\kappa$ 做精度分配。在 RMSNorm Transformer 中，激活方向集中在高奇异值子空间，与 $\kappa$ 敏感的最小奇异向量几乎正交，导致 $\kappa$ 系统性地高估误差且无预测能力。按 $\kappa$ 排序分配精度可能把高位资源放错位置。
...
2. 实验贡献：设计双指标评估协议互相验证；在统一平台上完成八种量化配置的横向对比，覆盖 FP8/FP4 两种格式和两个检查点，以分析误差传播机制。
3. 工程贡献：提供可复现的统一平台，包括模型、量化工具、分析工具和实验脚本。

### 7.3 局限

1. 模型规模受限。164M 参数远小于工业 LLM；GPTQ 在大模型上的表现可能不同，结论的泛化性需进一步验证。
